...
export default function Dashboard() {
  ...
  // ── 取引所APIキー連携 ────────────────────────────────
  const [credential,     setCredential]     = useState(null); // { exchange, is_verified, last_verified_at } | null
  ...
  const refreshCredentials = useCallback(async () => {
    try {
      ...
      // 一覧取得に失敗しても致命的ではないため握りつぶす（UIは「未設定」表示のまま）
    } finally {
  ...
  useEffect(() => {
    if (authChecked) refreshCredentials();
  }, [authChecked, refreshCredentials]);

  const handleSaveCredentials = useCallback(async (e) => {
    e.preventDefault();
    ...
    try {
      const res = await fetch('/api/credentials', {
        ...
        body: JSON.stringify({ exchange: 'bittrade', apiKey: apiKeyInput, apiSecret: apiSecretInput }),
      });
      const data = await res.json();
      if (!res.ok) {
        setCredMessage(data.error || '保存に失敗しました');
      } else {
        setCredMessage(data.message);
        setApiKeyInput('');
        setApiSecretInput('');
        await refreshCredentials();
      }
    } catch {
      setCredMessage('保存に失敗しました（通信エラー）');
    } finally {
      setCredSaving(false);
    }
  }, [apiKeyInput, apiSecretInput, refreshCredentials]);
  ...
  // 認証確認が終わるまでは何も描画しない（未ログイン状態でのチラ見え防止）
  if (!authChecked) {
    return (
      <div style={{
        background: '#0a0e1a', minHeight: '100vh', color: '#64748b',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: '#111827', border: '1px solid #1e2d45', borderRadius: 12 }}>
              <div style={{ padding: '14px 18px', borderBottom: '1px solid #1e2d45', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#00d4ff', letterSpacing: 1 }}>API KEY</span>
                {!credLoading && (
                  <span style={{
                    fontFamily: 'monospace', fontSize: 10, padding: '3px 8px', borderRadius: 10,
                    color: credential ? (credential.is_verified ? '#00ff9d' : '#ffd166') : '#64748b',
                    background: credential ? (credential.is_verified ? 'rgba(0,255,157,.1)' : 'rgba(255,209,102,.1)') : 'rgba(100,116,139,.1)',
                  }}>
                    {credential ? (credential.is_verified ? '検証済み' : '未検証') : '未設定'}
                  </span>
                )}
              </div>
              <form onSubmit={handleSaveCredentials} style={{ padding: 16 }}>
                <label style={{ display: 'block', fontSize: 10, color: '#64748b', letterSpacing: 1, marginBottom: 6, fontFamily: 'monospace' }}>
                  BITTRADE API KEY
                </label>
                <input
                  type="password"
                  ...
                  placeholder={credential ? '••••••••（変更する場合のみ入力）' : 'APIキーを入力'}
                  style={{ width: '100%', boxSizing: 'border-box', background: '#1a2235', border: '1px solid #1e2d45', borderRadius: 8, color: '#e2e8f0', fontFamily: 'monospace', fontSize: 12, padding: '9px 12px', marginBottom: 10, outline: 'none' }}
                />
                <label style={{ display: 'block', fontSize: 10, color: '#64748b', letterSpacing: 1, marginBottom: 6, fontFamily: 'monospace' }}>
                  BITTRADE API SECRET
                </label>
                <input
                  ...
                  placeholder={credential ? '••••••••（変更する場合のみ入力）' : 'APIシークレットを入力'}
                  style={{ width: '100%', boxSizing: 'border-box', background: '#1a2235', border: '1px solid #1e2d45', borderRadius: 8, color: '#e2e8f0', fontFamily: 'monospace', fontSize: 12, padding: '9px 12px', marginBottom: 12, outline: 'none' }}
                />
                <button
                  type="submit"
                  disabled={credSaving || !apiKeyInput || !apiSecretInput}
                  style={{
                    width: '100%', padding: '9px', borderRadius: 8, border: '1px solid #00d4ff',
                    background: 'rgba(0,212,255,.08)', color: '#00d4ff', fontFamily: 'monospace',
                    fontSize: 12, letterSpacing: 1, cursor: credSaving ? 'default' : 'pointer',
                    opacity: (credSaving || !apiKeyInput || !apiSecretInput) ? 0.5 : 1,
                  }}
                >
                  {credSaving ? '確認中...' : '保存 & 疎通確認'}
                </button>
                {credMessage && (
                  <div style={{ marginTop: 10, fontSize: 11, color: '#94a3b8', lineHeight: 1.5 }}>{credMessage}</div>
                )}
                <div style={{ marginTop: 10, fontSize: 10, color: '#475569', lineHeight: 1.5 }}>
                  キーはサーバー側でAES-256-GCM暗号化して保存されます。閲覧・再表示はできません。
                </div>
              </form>
            </div>
